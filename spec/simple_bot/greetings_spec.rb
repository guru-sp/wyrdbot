...
describe "SimpleBot::Greetings" do
  before :all do
    class Dummy; include SimpleBot::Greetings; end
  end
  ...
  context "when verifying the current part of the day" do
    it "should return morning for a 10:00" do
      allow(Time).to receive_message_chain(:now, :hour).and_return(10)
      expect(subject.day_part).to eql(:morning)
    end

    it "should return afternoon for 15:00" do
      allow(Time).to receive_message_chain(:now, :hour).and_return(15)
      expect(subject.day_part).to eql(:afternoon)
    end

    it "should return night for 23:00" do
      allow(Time).to receive_message_chain(:now, :hour).and_return(23)
      expect(subject.day_part).to eql(:night)
    end
  end

  it "shoud return false if it is not a greet" do
    expect(subject.greet?("ahh falo")).to be_falsey
  end

  it "shoud return true if it is a greet" do
    expect(subject.greet?("Bom dia")).to be_truthy
  end

  context "when greeting" do
    let(:hours){ "10:10" }
    let(:nick){ "Bill Gates" }
    let(:greet_responses){ YAML.load_file(File.expand_path(File.dirname(__FILE__))+"/../../talk_files/greetings.yml") }

    before { allow(Time).to receive_message_chain(:now, :strftime).and_return(hours) }

    context "in the morning" do
      before { expect(subject).to receive(:day_part).and_return(:morning) }
      ...
      it "should greet correctly with morning greet for a morning asking" do
        ...
        expect(response_sentence).to eql(resp)
      end
    ...
    context "in the afternoon" do
      before { expect(subject).to receive(:day_part).and_return(:afternoon) }
    ...
    context "in the night" do
      before { expect(subject).to receive(:day_part).and_return(:night) }
